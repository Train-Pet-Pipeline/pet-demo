...
def _track(tid: int, f: int) -> tuple[int, list[Track]]:
    return (f, [Track(track_id=tid, bbox=BBox(0, 0, 10, 10), score=1.0,
...
def test_simple_death_birth_within_gap_stitched():
    ...
    out = stitch_tracks(tracks, reids, cosine_threshold=0.9,
                        max_gap_frames=5, embedding_window=3)
...
def test_gap_exceeds_max_frames_no_stitch():
    ...
    out = stitch_tracks(tracks, reids, cosine_threshold=0.7,
                        max_gap_frames=10, embedding_window=3)
...
def test_empty_reids_is_noop_deep_copy():
    ...
    out = stitch_tracks(tracks, [], cosine_threshold=0.7,
                        max_gap_frames=5, embedding_window=3)
...
def test_hungarian_picks_best_pair_among_multiple_candidates():
    ...
    reids = [
        (0, {1: vec_a, 2: vec_b}), (1, {1: vec_a, 2: vec_b}), (2, {1: vec_a, 2: vec_b}),
        (5, {3: vec_a, 4: vec_b}), (6, {3: vec_a, 4: vec_b}),
    ]

    out = stitch_tracks(tracks, reids, cosine_threshold=0.9,
                        max_gap_frames=5, embedding_window=2)
